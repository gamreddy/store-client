...
export default function Cart({ cart, dispatch }) {
  ...
  const noItemsInCart = cart.reduce((total, item) => total + item.quantity, 0);

  return (
    <section id="cart">
      <h1>Cart: {noItemsInCart === 0 ? "Your cart is empty." : noItemsInCart === 1 ? "1 item in the cart." : noItemsInCart + " items in the cart."  }</h1>
